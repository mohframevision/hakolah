...
3. اختر المجموعة (مطاعم/متاجر/أدوات/أماكن)، اضغط **New** لإضافة عنصر جديد
   ...
   **الأيقونة والتصنيفات تُختار من قائمة جاهزة** (بدون كتابة يدوية) — القائمة معرّفة
   لكل قسم على حدة (شوف "تخصيص قائمة الأيقونات والتصنيفات" تحت).
4. اضغط **Save** ثم **Publish** — الموقع يتحدث تلقائياً خلال دقيقة أو دقيقتين (GitHub Action
   يبني وينشر تلقائياً).

### تخصيص قائمة الأيقونات والتصنيفات لكل قسم

من مجموعة **"الأقسام"** بلوحة التحكم، افتح القسم اللي تبيه، وبتلاقي حقلين:

- **الأيقونات المتاحة** — قائمة إيموجي تظهر كخيارات عند إضافة عنصر جديد بهذا القسم.
- **التصنيفات المتاحة** — قائمة تصنيفات (Tags) تظهر كخيارات بنفس الطريقة.

عبّيهم مرة وحدة لكل قسم، وبعدها أي عنصر جديد تضيفه (مطعم، متجر...) تختار أيقونته وتصنيفاته
من هالقوائم بدل ما تكتبهم من الصفر كل مرة. تقدر ترجع تضيف عليها لاحقاً وقت ما تحب.
...
### قسم بصفحات مقال مفصّلة لكل عنصر (مثل "أماكن")

بعض الأقسام (مثال: `أماكن`) كل عنصر فيها يفتح على صفحة مقال كاملة خاصة به (نبذة، طريقة
وصول، نصائح...) بدل بطاقة برابط خارجي بس. لتفعيل هذا لقسم جديد أو موجود: من مجموعة
"الأقسام"، فعّل خيار **"كل عنصر له صفحة مقال مفصّلة خاصة به"**. بعدها عناصر هذا القسم
بالضبط (مثل `src/places/karbabad-beach.md`) تكتب فيها المقال كامل بـ Markdown في
حقل "المقال الكامل"، وتُبنى له صفحة مستقلة تلقائياً على الرابط `<القسم>/<اسم-الملف>.html`.
...
**الصور تُضغط تلقائياً عند البناء** (`npm run build`، ونفس الشي يصير على GitHub Actions
عند كل رفع من لوحة التحكم): أي صورة برفعها بمجلد `assets/uploads` تُصغَّر تلقائياً لعرض
أقصى 1600px وتُضغط بجودة 80% بخطوة `scripts/minify.js` — النسخة المنشورة على الموقع
فقط، الصورة الأصلية اللي رفعتها تبقى محفوظة كما هي بالمستودع. يعني ما تحتاج تضغط الصورة
يدوياً قبل الرفع لأجل سرعة الموقع، **لكن** يفضّل تجنّب رفع صور ضخمة جداً (فوق 10-15
ميجابايت) لتقليل نمو حجم المستودع بمرور الوقت (جيتهاب المجاني يوصي بحد ~1GB للمستودع).
